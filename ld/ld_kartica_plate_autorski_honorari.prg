...
FUNCTION ld_kartica_plate_autorski_honorar( cIdRj, nMjesec, nGodina, cIdRadn, cObrac, aNeta )
   ...
   ? cMainLine
   ? cLDLijevaMargina + "1. BRUTO SA TROSKOVIMA :  ", AllTrim( Str( nBo ) ) + " * 1.2500 ="

   @ PRow(), 60 + Len( cLDLijevaMargina ) SAY nBSaTr PICT gpici

   ? cMainLine
   ? cLDLijevaMargina + "2. TROSKOVI", AllTrim( Str( gAhTrosk ) ) + "% (1 * troskovi%) ="

   @ PRow(), 60 + Len( cLDLijevaMargina ) SAY nTrosk PICT gpici

   ? cMainLine
   ? cLDLijevaMargina + "3. BRUTO BEZ TROSKOVA :  ", ld_bruto_isplata_ispis( nOsnZaBr, cRTipRada, nLicOdbitak )
   ...
   DO WHILE !Eof()
      ...
      ELSE
         nPom2 := get_benef_osnovica( _a_benef, dopr->idkbenef )
