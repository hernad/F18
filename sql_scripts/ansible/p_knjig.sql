...
CREATE TABLE IF NOT EXISTS {{ item_prodavnica }}.pos_knjig (
...
ALTER TABLE {{ item_prodavnica }}.pos_knjig OWNER TO admin;
CREATE INDEX IF NOT EXISTS pos_id1_knjig ON {{ item_prodavnica }}.pos_knjig USING btree (idpos, idvd, datum, brdok);
CREATE INDEX IF NOT EXISTS pos_id2_knjig ON {{ item_prodavnica }}.pos_knjig USING btree (idvd, datum);
CREATE INDEX IF NOT EXISTS pos_id3_knjig ON {{ item_prodavnica }}.pos_knjig USING btree (idPartner, datum);
CREATE INDEX IF NOT EXISTS pos_id6_knjig ON {{ item_prodavnica }}.pos_knjig USING btree (datum);
CREATE INDEX IF NOT EXISTS pos_knjig_dok_id ON {{ item_prodavnica }}.pos_knjig USING btree( dok_id );
GRANT ALL ON TABLE {{ item_prodavnica }}.pos_knjig TO xtrole;

CREATE TABLE IF NOT EXISTS {{ item_prodavnica }}.pos_items_knjig (
