...
FUNCTION kalk_get_1_rn()
   ...
   IF nRbr == 1  .OR. !kalk_is_novi_dokument()
      ...
      IF find_kalk_doks_by_broj_fakture( _IdVd, _BrFaktP, _IdKonto )
      //IF find_kalk_doks_by_broj_radnog_naloga( _IdFirma, _IdKonto, _brfaktp, "RN" )  // npr: 10-1200-564-RN, gdje je 1200 magacin gotovih proizvoda
         Beep( 2 )
         Msg( "Već postoji dokument RN broj " + kalk_doks->brdok + " za radni nalog:"  + _brfaktp )
         SELECT kalk_pripr
         KEYBOARD K_ESC
         nKalkStrana := 3
         RETURN  LastKey()
      ...
      find_kalk_doks_by_broj_fakture( _IdVd, _BrFaktP, _IdKonto2 )
      //find_kalk_doks_by_broj_radnog_naloga( _IdFirma, _IdKonto2, _brfaktp, NIL ) // 10-1100-564, 1100 - magacin sirovina, gdje su ovo ulazi
      GO TOP
      nII := 0
      nCntR := 0
      DO WHILE !Eof() // proci kroz sve kalk dokumente za odredjeni brfaktp - broj radnog naloga
